...
def compute_loss(logits, targets, valid_mask):
    """
    Args:
        logits: [B, N] predicted removal scores (lower = higher priority)
        targets: [B, N] ground truth removal ranks (0 = highest priority), -1 = ignored
        valid_mask: [B, N] mask of valid (unpadded) objects
    """
    # Only consider where target rank is defined (i.e., >= 0)
    mask = (targets >= 0) & valid_mask  # [B, N]
    
    if mask.sum() == 0:
        return torch.tensor(0.0, device=logits.device)  # prevent crash
    
    mse_loss = F.mse_loss(logits[mask], targets[mask].float())
    return mse_loss
