...
class Encoder(nn.Module):
    ...
    def forward(self, sequence):
        ...
        return hidden, cell, input_seq.shape[2]
...
class Decoder(nn.Module):
    ...
    def forward(self, dim, hidden, cell, actions=None):
        outputs = torch.zeros(self.args.sequence_length, self.args.batch_size, 1, IMAGE_SIZE, IMAGE_SIZE).to(self.device)
        decoder_input = torch.empty(1, self.args.batch_size, dim, dtype=torch.float, device=self.args.device)
