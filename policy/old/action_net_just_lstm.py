...
class ActionNet(nn.Module):
    def __init__(self, args, is_train=True):
        ...
        # self.encoder = nn.LSTM(input_size=output_dim1, hidden_size=self.hidden_size) # out, hid, cell = output_dim1, _, _
        self.decoder = nn.LSTM(input_size=output_dim1, hidden_size=self.hidden_size) # decoder(in, hid, cell) = output_dim1, output_dim1, output_dim1)

        self.out = nn.Linear(self.hidden_size, output_dim1)
    ...
    def checkpoint_forward(self, input, actions):
        ...
        decoder_hidden = (torch.zeros(1, self.args.batch_size, self.hidden_size, dtype=torch.float, device=self.args.device),
                          torch.zeros(1, self.args.batch_size, self.hidden_size, dtype=torch.float, device=self.args.device))
        
        decoder_input = torch.mean(input_seq, dim=0, keepdim=True)
