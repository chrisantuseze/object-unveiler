...
class FeatureTunk(nn.Module):
    ...
    def __init__(self, pretrained=False):
        ...
        self.dense121.conv0 = nn.Conv2d(2, 64, kernel_size=(7, 7), stride=(2, 2), padding=(3, 3), bias=False)

    def forward(self, depth, mask):

        concat = torch.cat((self.depth_extractor(depth), self.mask_extractor(mask)), dim=1)
        # print("concat.shape:", concat.shape)
        return self.dense121(concat)
