...
class SREDataset(data.Dataset):
    ...
    def __getitem__(self, id):
        scene_image, target_mask, object_masks, objects_to_remove, bboxes = self.memory.load_episode_sre(self.dir_ids[id])

        scene_image = general_utils.resize_mask(scene_image).mean(axis=2).astype(np.float32)
        target_mask = target_mask.astype(np.float32)

        object_masks = np.array(object_masks).astype(np.float32)

        scene_image = np.expand_dims(scene_image, axis=0)
        ...
        return scene_image, target_mask, padded_obj_masks, padded_bbox, objects_to_remove
    

    # For multi-object SRE
