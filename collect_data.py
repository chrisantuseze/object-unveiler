...
def collect_episodic_dataset(args):
    ...
    for i in range(args.n_samples):
        ...
        while node_id != target_id:
            ...
            target_obstacle = processed_masks[node_id]
            cv2.imwrite(os.path.join(TRAIN_DIR, "target_obstacle.png"), target_obstacle)

            state = policy.state_representation(obs)
            action = policy.guided_exploration(state, target_obstacle)
