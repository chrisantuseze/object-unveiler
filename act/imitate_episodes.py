...
from act.utils_act import load_data # data functions
from act.utils_act import sample_box_pose, sample_insertion_pose # robot functions
from act.utils_act import compute_dict_mean, set_seed, detach_dict # helper functions
...
def main(args):
    ...
    if is_eval:
        ckpt_names = [f'policy_epoch_200_seed_0.ckpt'] #[f'policy_best.ckpt']
        results = []
    ...
    train_dataloader, val_dataloader, stats, _ = load_data(config, dataset_dir, num_episodes, camera_names, batch_size_train, batch_size_val)
...
def eval_bc(config, ckpt_name, save_episode=True):
    ...
    # print("onscreen_render:", onscreen_render) #@Chris
...
def forward_pass(data, policy):
    ...
    image_data, action_data, is_pad = image_data.to(device), action_data.to(device), is_pad.to(device)
