...
class TagGraspNode:
    def __init__(self):
        ...
        #self.init_joints = [90.0, 150, 10, 10.0, 90, 90]
        #self.init_joints = [90.0, 90, 61, 20.0, 90, 90]
        self.init_joints = [90.0, 150, 12, 20.0, 90, 30]
        #self.init_joints = [90.0, 82, 90, 0.0, 90, 90]
        ...
        self.EndToCamMat = np.array([[1.00000000e+00,0.00000000e+00,0.00000000e+00,0.00000000e+00],
                                     [0.00000000e+00,1.00000000e+00,0,0],
                                     [0.00000000e+00,0,1.00000000e+00,0],
                                     [0.00000000e+00,0.00000000e+00,0.00000000e+00,1.00000000e+00]])
    ...
    def get_current_end_pos(self):
        ...
        if isinstance(response, kinemaricsResponse):
            ...
            self.CurEndPos[2] = response.z + 0.033
            self.CurEndPos[3] = response.Roll
            self.CurEndPos[4] = response.Pitch
            self.CurEndPos[5] = response.Yaw
            #print("_______________________________Current_End_Pose: ",self.CurEndPos)
    ...
    def tag_info_callback(self,msg):
        if self.dofbot_tracker.move_done == True and msg.z!=0:
            #print("******************************************************")
            center_x = msg.x
            center_y = msg.y
            depth = msg.z
            #self.dofbot_tracker.compute_Pose(center_x,center_y,depth)
            '''if abs(center_x-self.cur_x) <2 and  abs(center_y-self.cur_y) <2:
                self.cnt = self.cnt + 1
            self.cur_x = center_x
            self.cur_y = center_y'''
            if depth!=0 and self.dofbot_tracker.move_flag == True  and depth<0.70:
                #self.cnt = 0
                #print("000000000000000000000000000000000000000")
                self.dofbot_tracker.move_done = False
                #self.dofbot_tracker.compute_joint1(center_x)
                self.dofbot_tracker.Depth_track(center_x,center_y,depth)
                #self.dofbot_tracker.move_flag = False
            #print("Next move!")
        else:
            print("Moving now!")
        #self.dofbot_tracker.compute_Pose(center_x,center_y,depth)
    ...
    def grasp(self,pose_T):
        ...
        request.tar_y = pose_T[1] - 0.07 
        request.tar_z =  pose_T[2] - 0.04
        request.kin_name = "ik"
        request.Roll = ((2.5*request.tar_z*100)-207.5)*(math.pi / 180)
...
if __name__ == '__main__':
    try:
        ...
        tag_grasp.pubArm(tag_grasp.init_joints)
        rospy.spin()
