...
class TagGraspNode:
    ...
    def GetTargetIDCallback(self,msg):
        ...
        self.set_height = 0.03 + msg.data*0.01
    ...
    def tag_info_callback(self,msg):
        ...
        if self.set_height != 0:
            ...
            if pos_z!=0.0 :
                ...
                print("pose_T: ",pose_T)
                if self.grasp_flag == True and pose_T[0]<self.set_dist :
                    print("Discover the matching apriltag.")
                    self.grasp_flag = False
                    grasp = threading.Thread(target=self.grasp, args=(pose_T,))
                    grasp.start()
                    grasp.join()
                elif pose_T[0]>=self.set_dist and self.set_dist != 0:
