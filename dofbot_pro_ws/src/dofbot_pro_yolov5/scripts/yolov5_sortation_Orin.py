...
class Yolov5GraspNode:
    def __init__(self):
        ...
        self.toxic_waste=['Syringe','Expired_cosmetics','Expired_tablets','Used_batteries']
        self.wet_waste=['Fish_bone','Watermelon_rind','Apple_core','Egg_shell]
        self.dry_waste=['Toliet_paper','Disposable_chopsticks','Cigarette_butts','Peach_pit']
    ...
    def getSortFlagCallback(self,msg):
        ...
        if self.start_sort == True and self.name!=None and self.dist!=0:
            print("get the cx,cy",self.cx,self.cy)
            print("self.dist: ",self.dist)
            print("get the detect result",self.name)
            self.start_sort = False
            print("Start garbage sorting!")
    ...
    def move(self):
        ...
        if self.name in self.recyclable_waste:
            ...
            self.set_joint = [10.0, 60.0, 34.0, 16.0, 90.0,125]
        elif self.name in self.toxic_waste:
            
            print("This is toxic_waste.")
            self.set_joint = [30.0, 60.0, 34.0, 16.0, 90.0,125]
        elif self.name in self.wet_waste:
            
            print("This is wet_waste.")
            self.set_joint = [120.0, 60.0, 34.0, 16.0, 90.0,125]
        elif self.name in self.dry_waste:
            
            print("This is dry_waste.")
            self.set_joint = [140.0, 60.0, 34.0, 16.0, 90.0,125]
