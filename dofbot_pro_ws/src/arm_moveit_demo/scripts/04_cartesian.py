...
if __name__ == "__main__":
    ...
    dofbot_pro.set_planning_time(50)
    dofbot_pro.set_num_planning_attempts(20)
    ...
    #dofbot_pro.execute(dofbot_pro.plan())
    sleep(0.5)
    # 获取当前位姿数据最为机械臂运动的起始位姿
    end_effector_link = dofbot_pro.get_end_effector_link()
    start_pose = dofbot_pro.get_current_pose(end_effector_link).pose
    # 初始化路点列表
    waypoints = []
    # 如果为True,将初始位姿加入路点列表
    ...
    while fraction < 1.0 and attempts < maxtries:
        ...
        '''
        (plan, fraction) = dofbot_pro.compute_cartesian_path(waypoints, 0.1, 0.0, True)
        attempts += 1
        if attempts % 10 == 0:
            rospy.loginfo("Still trying after " + str(attempts) + " attempts...")
    if fraction == 1.0:
        rospy.loginfo("Path computed successfully. Moving the dofbot_pro.")
        dofbot_pro.execute(plan)
